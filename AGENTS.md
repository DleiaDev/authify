...
`authify` - a Next.js application. The problem statement and scope are not
defined yet; fill in `blueprint/project-plan.md` and `blueprint/build-plan.md`,
then run `/overview` to generate `blueprint/context/project-overview.md`.
...
No unit test runner is configured, so there is no required test gate yet. Run
`/tests` or `$tests` to add one and update this section with the real test
commands. There is no `Verify` command or automatic GitHub checks yet; run `/ci`
or `$ci` to set them up.
