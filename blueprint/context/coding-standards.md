...
> Your conventions. Tuned by `/onboard` to this project's stack: Next.js 16 (App
> Router, React Server Components), React 19, TypeScript 5 strict, Tailwind v4
> (CSS-first), shadcn/ui on Base UI, pnpm. Data layer, validation, and auth are
> not chosen yet and are marked `> TODO` below; revisit this file when they land.
...
- Server Actions: `actions/[feature].ts` (none yet)
- Types: `types/[feature].ts` (none yet)
...
> TODO: no database or ORM chosen yet. Decide the data layer (and its migration
> workflow) when the first data-backed feature is spec'd, then document it here.

## Data Fetching

- Server components fetch data directly; client components use Server Actions
- Validate all external input at the boundary (library not chosen yet, `> TODO`)
- Scope every user-owned query by the authenticated user id from the session on
  the server; never trust a client-supplied user id (auth provider `> TODO`)
